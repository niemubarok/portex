...
export function DocumentDetailModal({ document: providedDoc, documentId, onClose }: DocumentDetailModalProps) {
  ...
  useEffect(() => {
    if (selectedDoc && selectedDoc.poPath) {
          ...
          <div className="p-6 space-y-6 flex-1 overflow-y-auto">
            ...
            {selectedDoc.managerNotes && (
              <div className="p-3.5 rounded-xl bg-[var(--success)]/5 border border-[var(--success)]/20">
                ...
                <p className="text-sm text-[var(--text-secondary)] leading-relaxed italic">"{selectedDoc.managerNotes}"</p>
              </div>
            )}
            ...
            <div>
              ...
              <div className="space-y-2">
                <FileButton doc={selectedDoc} type="po" label="PO" path={selectedDoc.poPath} activeType={activePdfType} onClick={() => loadPdf(selectedDoc.id, 'po')} />
                <FileButton doc={selectedDoc} type="invoice" label="Invoice" path={selectedDoc.invoicePath} activeType={activePdfType} onClick={() => loadPdf(selectedDoc.id, 'invoice')} />
                <FileButton doc={selectedDoc} type="packing_list" label="Packing List" path={selectedDoc.packingListPath} activeType={activePdfType} onClick={() => loadPdf(selectedDoc.id, 'packing_list')} />
                <FileButton doc={selectedDoc} type="peb" label="PEB" path={selectedDoc.pebPath} activeType={activePdfType} onClick={() => loadPdf(selectedDoc.id, 'peb')} />
                <FileButton doc={selectedDoc} type="bl" label="Bill of Lading" path={selectedDoc.blPath} activeType={activePdfType} onClick={() => loadPdf(selectedDoc.id, 'bl')} />
                <FileButton doc={selectedDoc} type="other" label="Lainnya" path={selectedDoc.otherPath} activeType={activePdfType} onClick={() => loadPdf(selectedDoc.id, 'other')} />
              </div>
            </div>
